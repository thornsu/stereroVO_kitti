...
{
    boost::timer timer;
    orb_->detect ( curr_->img_left_, keypoints_curr_left_ );
...
{
    boost::timer timer;
    orb_->compute ( curr_->img_left_, keypoints_curr_left_, descriptors_curr_left_);
    ...
    if ( map_->keyframes_.empty() )
    {
        ...
        for ( size_t i=0; i<pt2.size(); i++ )
        {
            if(status[i])
            {
                ...
                //double d = curr_->findDepth ( keypoints_curr_left_[i] );
